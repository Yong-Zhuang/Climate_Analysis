...
def get_samples(lead,look,is3d,normalization):
    ...
    ino = np.load(path_data+'persiann_1_x_1_look15.npy')
    ...
    X_tr = [Pob[0:train_validate],Px_mean[0:train_validate],Px_spread[0:train_validate],Qx[0:train_validate]] 
    #X_val = [Px_mean[test_train:train_validate],Px_spread[test_train:train_validate],Qx[test_train:train_validate]] 
    X_te = [Pob[train_validate:],Px_mean[train_validate:],Px_spread[train_validate:],Qx[train_validate:]] 
    ...
    Y = np.square(Qy)
    sq_Y_tr = Y[0:train_validate]
    #Y_val =  Qy[test_train:train_validate]
    sq_Y_te = Y[train_validate:]  
     
    f_conf = (lead, 1, map_height, map_width) if lead > 0 else None
    o_conf = (look, 1, map_height, map_width) if look > 0 else None
    return X_tr,X_te,sq_Y_tr,sq_Y_te,Y_tr,Y_te,o_conf, f_conf,external_dim
def init_fun(args):  
    lr = args.lr
...
if __name__== "__main__": 
    ...
    X_tr,X_te,sq_Y_tr,sq_Y_te,Y_tr,Y_te,o_conf, f_conf,external_dim = get_samples(args.lead,args.look,args.is3d,args.norm)
    ...
    hyperparams_name = 'test_dms.{}.{}.{}'.format(args.model,args.lead,args.look)
    ...
    callbacks = [model_checkpoint, early_stopping,reduce_lr,predcall,tb]#,tb
    
    
    history = model.fit(X_tr, sq_Y_tr,
                epochs=nb_epoch,
                batch_size=batch_size,
                validation_data=(X_te,sq_Y_te),
                callbacks=callbacks,
                verbose=1)
