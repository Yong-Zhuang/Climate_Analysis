...
if __name__== "__main__": 
    ...
    X,Y,E,logY = Normalization(X,Y,E,train_idx,args.norm)
        
    E_train, X_train, Y_train, E_test, X_test, Y_test,logY_tr,logY_te = E[train_idx], X[train_idx],Y[train_idx],E[test_idx],X[test_idx],Y[test_idx] ,logY[train_idx],logY[test_idx]    
    ...
    predcall = PredictionCallback(X_te,Y_test)
    ...
    history = model.fit(X_tr, logY_tr, epochs=nb_epoch, verbose=1, batch_size=batch_size, callbacks=[model_checkpoint, predcall,tb], validation_data=(X_te, logY_te))#early_stopping,
    ...
    m_mae,m_rmse,m_r2 = get_metrics(np.array(Y_train).flatten(),np.array(prediction).flatten())
    ...
    m_mae,m_rmse,m_r2 = get_metrics(np.array(Y_test).flatten(),np.array(prediction).flatten())
    print('Test score: %.6f rmse (norm): %.6f r_squared: %.6f' % (m_mae, m_rmse,m_r2))
    pickle.dump((prediction), open(os.path.join(
