...
def NASNet(input_shape=None,
    ...
    if not initial_reduction:  # imagenet / mobile mode
        if use_auxiliary_branch:
            auxiliary_x = _add_auxiliary_head(x, weight_decay, pooling)
...
def _add_auxiliary_head(x, weight_decay, pooling):
...
def net(o_conf=(15, 1, 32, 32),f_conf=(15, 1, 32, 32), external_dim=5,mode=18):#121 169 201 264 161
    ...
    if mode=='large':
        nas_net0 = NASNetLarge(input_shape=o_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net1 = NASNetLarge(input_shape=f_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net2 = NASNetLarge(input_shape=f_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
    elif mode=='mobile':
        nas_net0 = NASNetMobile(input_shape=o_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net1 = NASNetMobile(input_shape=f_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net2 = NASNetMobile(input_shape=f_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
    else:# mode=='cifar':
        nas_net0 = NASNetCIFAR(input_shape=o_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net1 = NASNetCIFAR(input_shape=f_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net2 = NASNetCIFAR(input_shape=f_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
    for layer in nas_net0.layers:
      layer.trainable = True
    x0 = nas_net0.output
    for layer in nas_net1.layers:
      layer.name = layer.name + str("_1")
      layer.trainable = True
    x1 = nas_net1.output
    for layer in nas_net2.layers:
    ...
    #print (f.shape)   
    model = Model(inputs=[nas_net0.input,nas_net1.input,nas_net2.input,input3], outputs=f)   
...
def net_v2(o_conf=(15, 1, 32, 32),f_conf=(15, 1, 32, 32), external_dim=5,mode=18):#121 169 201 264 161
    ...
    dp = None
    uab = False
    input1 = Input(shape=(f_channels, lead, f_height, f_width)) 
    input2 = Input(shape=(f_channels, lead, f_height, f_width)) 
    input_fusion = concatenate([input1, input2],axis = 1)
    
    if mode=='large':
        nas_net0 = NASNetLarge(input_shape=o_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net1 = NASNetLarge(input_tensor=input_fusion, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
    elif mode=='mobile':
        nas_net0 = NASNetMobile(input_shape=o_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net1 = NASNetMobile(input_tensor=input_fusion, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
    else:# mode=='cifar':
        nas_net0 = NASNetCIFAR(input_shape=o_shape, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
        nas_net1 = NASNetCIFAR(input_tensor=input_fusion, pooling=pl,dropout=dp,use_auxiliary_branch = uab)
    for layer in nas_net0.layers:
        layer.trainable = True
    x0 = nas_net0.output
    for layer in nas_net1.layers:
    ...
    f = fusion.net_v2(x0,x1,x2)
    
    model = Model(inputs=[nas_net0.input,input1, input2,x2], outputs=f)   
