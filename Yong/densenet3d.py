...
def DenseNetImageNet121(input_shape=None,
    ...
    return DenseNet(input_shape, depth=121, nb_dense_block=4, growth_rate=32, nb_filter=64,
                    ...
                    dropout_rate=dropout, weight_decay=weight_decay, subsample_initial_block=True,
...
def __transition_block(ip, nb_filter, compression=1.0, weight_decay=1e-4, block_prefix=None, transition_pooling='max'):
    ...
    with K.name_scope('Transition'):
        ...
        if transition_pooling == 'avg':
            print (x.shape)
            x = AveragePooling3D((1, 2, 2), strides=(2, 2, 2))(x)
...
def __create_dense_net(img_input, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
                       ...
                       subsample_initial_block=False, pooling=None, transition_pooling='avg'):
...
def __create_fcn_dense_net(nb_classes, img_input, include_top, nb_dense_block=5, growth_rate=12,
    ...
    with K.name_scope('DenseNetFCN'):
        ...
        return x
def net(o_conf=(15, 1, 32, 32),f_conf=(15, 1, 32, 32), external_dim=5,mode=121):#121 169 201 264 161
    lead, f_channels, f_height, f_width = f_conf
    look, o_channels, o_height, o_width = o_conf
    pl = 'max'
    if mode==121:
        dense_net0 = DenseNetImageNet121(input_shape=( o_channels, look, o_height, o_width),
                                         pooling=pl)
        dense_net1 = DenseNetImageNet121(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
        dense_net2 = DenseNetImageNet121(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
    elif mode==169:
        dense_net0 = DenseNetImageNet169(input_shape=( o_channels, look, o_height, o_width),
                                         pooling=pl)
        dense_net1 = DenseNetImageNet169(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
        dense_net2 = DenseNetImageNet169(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
    elif mode==201:
        dense_net0 = DenseNetImageNet201(input_shape=( o_channels, look, o_height, o_width),
                                         pooling=pl)
        dense_net1 = DenseNetImageNet201(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
        dense_net2 = DenseNetImageNet201(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
    elif mode==264:
        dense_net0 = DenseNetImageNet264(input_shape=( o_channels, look, o_height, o_width),
                                         pooling=pl)
        dense_net1 = DenseNetImageNet264(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
        dense_net2 = DenseNetImageNet264(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
    else:
        dense_net0 = DenseNetImageNet161(input_shape=( o_channels, look, o_height, o_width),
                                         pooling=pl)
        dense_net1 = DenseNetImageNet161(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
        dense_net2 = DenseNetImageNet161(input_shape=( f_channels, lead, f_height, f_width),
                                         pooling=pl)
    for layer in dense_net0.layers:
      layer.trainable = True
    x0 = dense_net0.output
    for layer in dense_net1.layers:
      layer.name = layer.name + str("_1")
      layer.trainable = True
    x1 = dense_net1.output
    for layer in dense_net2.layers:
      layer.name = layer.name + str("_2")
      layer.trainable = True
    x2 = dense_net2.output
    ...
    model = Model(inputs=[dense_net0.input,dense_net1.input,dense_net2.input,input3], outputs=f)   
...
def net_mt(o_conf=(15, 1, 32, 32),f_conf=(15, 1, 32, 32), external_dim=5,mode=18):#121 169 201 264 161
    ...
    if mode==121:
        net0 = DenseNetImageNet121(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet121(input_tensor=input_fusion, pooling=pl,dropout=dp)
    elif mode==169:
        net0 = DenseNetImageNet169(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet169(input_tensor=input_fusion, pooling=pl,dropout=dp)
    elif mode==201:
        net0 = DenseNetImageNet201(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet201(input_tensor=input_fusion, pooling=pl,dropout=dp)
    elif mode==264:
        net0 = DenseNetImageNet264(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet264(input_tensor=input_fusion, pooling=pl,dropout=dp)
    else:
        net0 = DenseNetImageNet161(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet161(input_tensor=input_fusion, pooling=pl,dropout=dp)
    ...
    model = Model(inputs=[net0.input,input1,input2,x3,x4,x5], outputs=f)     
...
def net_mt_v1(o_conf=(15, 1, 32, 32),f_conf=(15, 1, 32, 32), external_dim=5,mode=18):#121 169 201 264 161
    ...
    if mode==121:
        net0 = DenseNetImageNet121(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet121(input_shape=f_shape, pooling=pl,dropout=dp)
        net2 = DenseNetImageNet121(input_shape=f_shape, pooling=pl,dropout=dp)
    elif mode==169:
        net0 = DenseNetImageNet169(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet169(input_shape=f_shape, pooling=pl,dropout=dp)
        net2 = DenseNetImageNet169(input_shape=f_shape, pooling=pl,dropout=dp)
    elif mode==201:
        net0 = DenseNetImageNet201(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet201(input_shape=f_shape, pooling=pl,dropout=dp)
        net2 = DenseNetImageNet201(input_shape=f_shape, pooling=pl,dropout=dp)
    elif mode==264:
        net0 = DenseNetImageNet264(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet264(input_shape=f_shape, pooling=pl,dropout=dp)
        net2 = DenseNetImageNet264(input_shape=f_shape, pooling=pl,dropout=dp)
    else:
        net0 = DenseNetImageNet161(input_shape=o_shape, pooling=pl,dropout=dp)
        net1 = DenseNetImageNet161(input_shape=f_shape, pooling=pl,dropout=dp)
        net2 = DenseNetImageNet161(input_shape=f_shape, pooling=pl,dropout=dp)
    for layer in net0.layers:
        layer.trainable = True
    x0 = net0.output
    for layer in net1.layers:
    ...
    x4 = Input((external_dim,))
    x5 = Input((external_dim,))
    f = fusion.net_mt3(x0,x1,x2,x3,x4,x5)
    
    model = Model(inputs=[net0.input,net1.input,net2.input,x3,x4,x5], outputs=f)    
    return model